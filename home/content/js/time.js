...
function Time (h,m,s,ms,a) {
  ...
  this.millis = ((H * hour) + (M * min) + (S * sec) + (MS * mils) + ((meridian && (meridian.toLowerCase().charAt(0) === 'p')) ? PM : AM)) % DAY;
} //Time
...
Time.prototype = {
  ...
  addHours: function (h) {
    this.millis = (this.millis + (h * H)) % DAY;
    return this;
  }, //addHours
  ...
  addMinutes: function (m) {
    this.millis = (this.millis + (m * M)) % DAY;
    return this;
  }, //addMinutes
  ...
  addSeconds: function (s) {
    this.millis = (this.millis + (s * S)) % DAY;
    return this;
  }, //getSeconds
  ...
  addMilliseconds: function (ms) {
    this.millis = (this.millis + ms) % DAY;
    return this;
  }, //addMilliseconds
